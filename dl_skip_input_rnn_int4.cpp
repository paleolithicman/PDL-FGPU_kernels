 ...
 */
#define N_SIZE              4096
#define DATA_SIZE_INV       2
#define NUM_MATS            1

#define N_BYTES             (N_SIZE / DATA_SIZE_INV)
...
#define NUM_ELEMS_PER_BLK   (NUM_VREGS_PER_BLK * sizeof(uint8x128) * DATA_SIZE_INV)

__kernel void skip_input_rnn_int4(int iters, __global int8_t* h, __global int8_t* x, __global int8_t* A)
{
    __local int8_t* h_shr_ptr = 0;                                      // N_BYTES
    __local int* swp_shr_ptr  = (__local int*) (h_shr_ptr + N_BYTES);   // NUM_WG_THDS

    int gid = get_global_id(0);
    int lid = get_local_id(0);

    int lid_cmb_1  = lid ^ 0x0001;
    int row_parity = gid & 0x0001;

    __global int8_t* A_ptr = A + (gid * NUM_ELEMS_PER_BLK / DATA_SIZE_INV);
    ...
    for (int i = 0; i < iters; ++i) {
        ...
        /* add input */
        res += ((x_ptr[gid >> 1] << (24 + (4 * (row_parity ^ 1)))) >> 28);
        ...
        swp_shr_ptr[lid] = res;
        res = ((swp_shr_ptr[lid_cmb_1] & 0xf) << 4) | (res & 0xf);

        global_sync();

        /* store result */
        store_if_zero_int8(res, row_parity, &h[gid >> 1], 0);
